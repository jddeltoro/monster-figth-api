...
describe('MonsterController', () => {
  ...
  describe('Import CSV', () => {
    test('should fail when importing csv file with an empty monster', async () => {
      const filePath = path.join(process.cwd(), 'data', 'monsters-empty-monster.csv');
      const fileBuffer = fs.readFileSync(filePath);
      const response = await request(server)
        .post('/monsters/import')
        .attach('monsters', fileBuffer, 'monsters-empty-monster.csv');
      expect(response.status).toBe(StatusCodes.BAD_REQUEST);
    });

    test('should fail when importing csv file with wrong or inexistent columns.', async () => {
      const filePath = path.join(process.cwd(), 'data', 'monsters-wrong-column.csv');
      const fileBuffer = fs.readFileSync(filePath);
      const response = await request(server)
        .post('/monsters/import')
        .attach('monsters', fileBuffer, 'monsters-wrong-column.csv');
      expect(response.status).toBe(StatusCodes.BAD_REQUEST);
    });

    test('should import all the CSV objects into the database successfully', async () => {
      const filePath = path.join(process.cwd(), 'data', 'monsters-correct.csv');
      const fileBuffer = fs.readFileSync(filePath);
      const response = await request(server)
        .post('/monsters/import')
        .attach('monsters', fileBuffer, 'monsters-wrong-column.csv');
